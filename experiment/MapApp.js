var MapApp = Class.extend({
    ...
    /////////////////////////////////////////////////////////////
	//WHAT I ADDED
	DivvyLayer: function(data, aNewLayer) { 
		var divvydata = data;
		var stationsInfo = [];
		for(var i = 0; i < divvydata.length; i++)
                   {
                       var lon = parseFloat(divvydata[i].longitude);
                       var lat = parseFloat(divvydata[i].latitude);
                       var name = (divvydata[i].stationName);
                       var dcapacity = parseInt(divvydata[i].totalDocks);
                       var stationID = parseInt(divvydata[i].id);
					   var acapacity = parseInt(divvydata[i].availableBikes);
                      // var popOfStation = parseFloat(popular[i].Total);
                      //all information about specefic stations stored here 
                      stationsInfo[i] = [name, lat, lon, dcapacity, stationID, acapacity];
                      //popularity[i] = popOfStation; //seperate array needed to scale for opacity                                           
                   }
		for(var i = 0; i < divvydata.length; i++)
                   {
                   //adding stations to a layerGroup instead of straigh to the map for toggabiity (Not a word, apparently)

                        var marker = L.marker([stationsInfo[i][1], stationsInfo[i][2]],{/*icon: myIcon,*/ /*opacity: (scale(stationsInfo[i][5])) ,*/'marker-color': '#ff8888', riseOnHover: true})
                              .addTo(aNewLayer).bindPopup(stationsInfo[i][0] + "<br/>Total Docks: " + stationsInfo[i][3] + "<br/>Available Bikes: " + stationsInfo[i][5]);

            
                        marker["name"] = stationsInfo[i][0];
						marker["stationID"] = stationsInfo[i][4];
                        marker["lat"] = stationsInfo[i][1];
                        marker["lon"] = stationsInfo[i][2];
					
					}

    //console.log(data.query.results.json.stationBeanList);
	},
	
	DivvyData: function(data, aNewLayer)
	{
		var divvydata = data.stationBeanList;
		console.log(divvydata);
		var test = aNewLayer;
		console.log(test);
		this.DivvyLayer(divvydata, aNewLayer);
	},
	
	DivvyJson: function(site, aNewLayer)
	{
		requestCrossDomainJSON(site, this.DivvyData);
    ...
    refreshData: function()
    {
		...
		//WHAT I ADDED
		this.DivvyJson("http://divvybikes.com/stations/json/",this.layers[3]);
		//WHAT I ADDED
        ...
        var overlays = {
			...
			"Divvy Station"			: this.layers[3], //WHAT I ADDED
        };
